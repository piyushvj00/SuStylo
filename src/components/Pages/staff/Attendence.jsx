...
const Attendance = () => {
  ...
  const checkTodayAttendance = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/attendance/staff/${staffId}`);
      const attendance = response.data.attendances?.[0] || null;
  ...
  const handleMarkIn = async () => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || 'Failed to mark in';
  ...
  const handleMarkOut = async () => {
    try {
      ...
      const response = await axiosInstance.put(
        `/attendance/staff/${staffId}/markout`
      );
      ...
      const errorMessage =
        error.response?.data?.message || 'Failed to mark out';
    ...
    <div className="p-2">
      ...
      <div className="md:flex items-center justify-between">
        ...
        <div className="space-x-4 mb-6">
          ...
          <button
            onClick={handleMarkIn}
            disabled={attendanceData?.markIn || loading}
            className={`py-2 px-5 rounded-xl text-white font-semibold transition
              ${
                attendanceData?.markIn
                  ? 'bg-gray-300 cursor-not-allowed'
                  : 'bg-green-600 hover:bg-green-700'
              }`}
          >
            {loading && !attendanceData?.markIn
              ? 'Processing...'
              : 'Mark In'}
          </button>
          ...
          <button
            ...
            className={`py-2 px-5 rounded-xl text-white font-semibold transition
              ${
                !attendanceData?.markIn || attendanceData?.markOut
                  ? 'bg-gray-300 cursor-not-allowed'
          ...
          <div className="p-4 rounded-xl mb-6 space-y-3">
            <div className="flex gap-3">
              ...
              <span className="text-green-800">
                {formatTime(attendanceData.markIn)} (
            ...
            <div className="flex gap-3">
              ...
              <span className="text-green-800">
                {attendanceData.markOut
                  ? formatTime(attendanceData.markOut)
                  : 'N/A'}
              </span>
            </div>
