...
const AllComments = () => {
  ...
  const fetchAllComments = React.useCallback(async () => {
    try {
      ...
      // Fetch comments for each blog using different endpoints
      for (const blog of blogs) {
        try {
          // Try to get comments for this blog
          const endpoints = [
            `/blogs/${blog._id}/comments`,
            `/blogs/approved/${blog._id}`,
            `/blogs/pending/${blog._id}`,
            `/blogs/rejected/${blog._id}`
          ];
          
          const commentPromises = endpoints.map(endpoint => 
          ...
          results.forEach((result, index) => {
            if (result.data.success && Array.isArray(result.data.data)) {
              ...
              const blogComments = result.data.data.map(comment => ({
                ...comment,
                status: status || comment.status || 'pending',
                blogTitle: blog.title,
                blogSlug: blog.slug,
                blogId: blog._id
              }));
